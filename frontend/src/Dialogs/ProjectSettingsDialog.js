...
class ProjectSettingsDialog extends React.Component {
  ...
  onConfirm = accept => () => {
    if (accept) {
      // New settings to parent
      this.props.onSettingsChange(this.state)
    } else {
      // Retrieve old settings from parent
  ...
  componentWillReceiveProps(newProps) {
    if (!newProps.open) this.setState(newProps.settings)
  }
  ...
  handleNextAlignChange = evt => {
    this.setState({ bbNextAlign: evt.target.value })
  }
